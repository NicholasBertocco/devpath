...
const CodeSandboxService_1 = require("../services/CodeSandboxService");
...
const createSubmission = async (req, res) => {
    try {
        ...
        for (const testCase of exercise.testCases) {
            const result = CodeSandboxService_1.CodeSandboxService.execute(exercise.language, code, testCase.input, testCase.expectedOutput);
            if (!result.passed) {
        ...
        if (allTestsPassed) {
            const previousSuccess = await prisma_1.prisma.submission.findFirst({
                where: {
                    userId,
                    exerciseId,
                    status: 'SUCCESS',
            ...
            if (!previousSuccess) {
                ...
                if (user) {
                    ...
                    const successCount = await prisma_1.prisma.submission.count({
                        where: { userId, status: 'SUCCESS' }
                    });
                    if (successCount === 1) { // Acabou de salvar a primeira, então a contagem no banco é 1
                        let firstBadge = await prisma_1.prisma.badge.findFirst({ where: { name: 'Primeiro Passo' } });
                        // Se a badge ainda não existir no banco, nós a criamos dinamicamente (para facilitar o MVP)
                        if (!firstBadge) {
                            firstBadge = await prisma_1.prisma.badge.create({
                                data: {
                                    name: 'Primeiro Passo',
                    ...
                    if (exercise.language === 'javascript') {
                        const jsSuccessCount = await prisma_1.prisma.submission.count({
                            where: {
                                userId,
                                status: 'SUCCESS',
                        ...
                        if (jsSuccessCount === 3) {
                            let jsBadge = await prisma_1.prisma.badge.findFirst({ where: { name: 'Mestre do JavaScript' } });
                            if (!jsBadge) {
                                jsBadge = await prisma_1.prisma.badge.create({
